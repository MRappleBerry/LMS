...
export async function explainWithAi({ context, instruction }) {
  ...
  try {
    const { data } = await axios.post(`${API_BASE}/api/explain`, payload, { headers: getAuthHeaders(), withCredentials: true })
    return data
  } catch (err) {
    // Fallback for older backend deployments.
    if (err.response?.status === 404) {
      const legacyPrompt = `${instruction}\n\nContext:\n${context}`
      const { data } = await axios.post(`${API_BASE}/api/chat`, { message: legacyPrompt }, { headers: getAuthHeaders(), withCredentials: true })
